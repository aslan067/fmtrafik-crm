...
export default function EditProductPage() {
  ...
  // Tedarikçi veya fiyat parametreleri değiştikçe hesaplamayı tetikle
  useEffect(() => {
    if (!loading) {
      calculatePrices()
    }
  ...
  async function loadInitialData() {
    try {
      ...
      setFormData({
        ...
        // Veritabanından gelen veya varsayılan çarpan
        price_multiplier: product.price_multiplier || '1.00', 
  ...
  function calculatePrices() {
    ...
    // 1. Tedarikçi Tipine Göre Hesaplama
    if (supplier.discount_type === 'percentage') {
      // SENARYO B: Liste Fiyatı + İskonto
      // Bayi liste fiyatı genelde tedarikçi liste fiyatıyla aynıdır (veya manuel girilir)
      // Otomatik hesaplamada burayı liste fiyatına eşitliyoruz
    ...
    // Sadece otomatik hesaplanan değeri öneriyoruz
    // Kullanıcı manuel olarak değiştirebilir, ancak parametreler değiştiğinde burası tetiklenir.
    setFormData(prev => ({
      ...prev,
  ...
  const handleSupplierChange = (e) => {
    ...
    setFormData(prev => ({
      ...
      // Tedarikçi değişince o tedarikçinin varsayılan değerlerini getir
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const { error: updateError } = await supabase.from('products').update({
        ...
        price_multiplier: parseFloat(formData.price_multiplier) || 1.0, // Çarpanı kaydet
  ...
  // UI Hesaplamaları (Anlık Görüntüleme İçin)
          ...
          <div className="mb-6 flex items-center justify-between">
            ...
            <button type="button" onClick={() => {
               if(confirm('Silmek istediğinize emin misiniz?')) {
                 supabase.from('products').delete().eq('id', params.id).then(() => router.push('/products'))
               }
            }} className="text-red-600 hover:bg-red-50 px-3 py-2 rounded transition-colors flex items-center gap-2">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-2 space-y-6">
                ...
                {/* Fiyatlandırma (DİNAMİK BÖLÜM) */}
                <div className="card border-l-4 border-l-blue-500">
                  <h3 className="font-semibold mb-4 flex items-center gap-2"><Calculator className="w-5 h-5 text-blue-600"/> Fiyatlandırma Stratejisi</h3>
                    ...
                    <div className="bg-gray-50 p-4 rounded-lg border border-gray-200 space-y-4">
                      <div className="flex items-center justify-between">
                        ...
                        <div className="relative w-1/2">
                          ...
                          <input type="number" name="supplier_list_price" value={formData.supplier_list_price} onChange={handleChange} className="input-field pl-8 font-bold" />
                        </div>
                      </div>
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <div className="relative w-1/2">
                          {selectedSupplier.discount_type === 'percentage' ? (
                            <>
                              <input type="number" name="supplier_discount_percentage" value={formData.supplier_discount_percentage} onChange={handleChange} className="input-field pr-8" />
                              <span className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-500">%</span>
                            </>
                          ) : (
                            <>
                              <span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500 font-bold">x</span>
                              <input type="number" name="price_multiplier" value={formData.price_multiplier} onChange={handleChange} className="input-field pl-8" step="0.01" />
                            </>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="pt-4 border-t border-gray-300 flex items-center justify-between">
                        ...
                        <div className="relative w-1/2">
                          <span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500">{currencySymbols[formData.currency]}</span>
                          <input type="number" name="dealer_list_price" value={formData.dealer_list_price} onChange={handleChange} className="input-field pl-8 font-extrabold text-lg text-blue-600 border-blue-300" />
                        </div>
                      </div>
                    </div>
                  ) : (
            ...
            <div className="flex justify-end gap-3">
              ...
              <button type="submit" disabled={saving || uploading} className="btn-primary flex items-center gap-2">{saving ? 'Kaydediliyor...' : <><Save className="w-4 h-4"/> Güncelle</>}</button>
            </div>
          </form>
        </div>
      </div>
