...
import { 
  ...
  Check, RefreshCw, User, Copy, ChevronDown, Settings, Ruler, Lock, Unlock, AlertTriangle, X
} from 'lucide-react'
...
export default function EditQuotePage() {
  ...
  const [formData, setFormData] = useState({
    customer_id: '',
    contact_id: '',
    quote_number: '', // Sadece gösterim için
    title: '',
    valid_until: '',
    currency: 'TRY',
    exchange_rate: 1.00,
  ...
  // Kur değişince hesapla (Yükleme bittikten sonra)
  useEffect(() => {
    if (!loading && items.length > 0) recalculateAllItems()
  ...
  async function loadInitialData() {
    try {
      ...
      setFormData({
        ...
        exchange_rate: parseFloat(quote.exchange_rate) || 1.00,
        template_code: quote.template_code || 'standard_tr',
        selected_bank_ids: quote.selected_bank_ids || [],
        show_product_images: quote.show_product_images,
        show_specifications: quote.show_specifications,
        notes: quote.notes || '',
        terms: quote.terms || '',
        default_tax_rate: 20, // Varsayılan
        discount_percentage: parseFloat(quote.discount_percentage) || 0
      })

      // Kalemleri Doldur (DB'den gelen verileri State formatına uyarla)
      const formattedItems = quoteItems.map(item => ({
        ...item,
        id: item.id, // DB ID'si kalsın (Silme/Ekleme yapacağımız için aslında çok kritik değil ama key için iyi)
        ...
        original_list_price: parseFloat(item.original_list_price) || 0,
        original_cost_price: parseFloat(item.original_cost_price) || 0,
        // Eğer ürün fiyatı 0 ise veya manuel girildiyse kilidi aç
        is_price_editable: parseFloat(item.list_price) <= 0 || !item.product_id
      }))
  ...
  // --- Hesaplama Motoru (New Quote ile Aynı) ---
  ...
  const cloneItem = (index) => {
    ...
    const newItem = { ...itemToClone, id: Date.now() } // Yeni ID ver (DB'den bağımsız)
    const newItems = [...items]
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // 2. Kalemleri Temizle (Eskileri Sil)
      ...
      const quoteItems = items.map((item, idx) => ({
        ...
        original_list_price: item.original_list_price,
        original_cost_price: item.original_cost_price,
      ...
      <div className="p-6 min-h-screen bg-gray-50/50">
        <div className="max-w-[1600px] mx-auto">
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              <div className="flex items-center gap-3">
                ...
                <span className="bg-blue-100 text-blue-700 px-3 py-1 rounded-full text-sm font-bold">{formData.quote_number}</span>
              </div>
              <p className="text-gray-600 mt-1">Teklif detaylarını güncelleyin.</p>
            </div>
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 xl:grid-cols-12 gap-6">
                      ...
                      {bankAccounts.map(b => (
                        <label key={b.id} className="flex items-center gap-3 p-3 hover:bg-blue-50 cursor-pointer border-b last:border-0 border-gray-50 transition-colors">
                          <input 
                            ...
                            onChange={() => {
                              const current = formData.selected_bank_ids || []
                              if (current.includes(b.id)) setFormData(prev => ({ ...prev, selected_bank_ids: current.filter(id => id !== b.id) }))
                              else setFormData(prev => ({ ...prev, selected_bank_ids: [...current, b.id] }))
                            }}
            ...
            {/* SAĞ KOLON (9/12) - Ürün Listesi (New Quote ile Birebir Aynı Yapı) */}
                ...
                <div className="flex justify-between items-center mb-4">
                  ...
                  <div className="flex items-center gap-3">
                    <div className="flex items-center gap-2 bg-gray-100 px-3 py-1 rounded-lg border border-gray-200">
                      ...
                      <input type="number" value={formData.default_tax_rate} onChange={(e) => setFormData({...formData, default_tax_rate: parseFloat(e.target.value) || 0})} className="w-12 bg-transparent font-bold text-sm text-center outline-none border-b border-gray-300 focus:border-blue-500" />
                          ...
                          <tr key={item.id} className={`group transition-colors ${isZeroPrice ? 'bg-red-50' : 'hover:bg-blue-50/50'}`}>
                            ...
                            <td className="p-2 relative group/price">
                              <div className="relative">
                                ...
                                />
                                <button type="button" onClick={() => togglePriceLock(index)} className="absolute right-2 top-1/2 -translate-y-1/2 text-gray-400 hover:text-blue-600 transition-colors">
                                  {item.is_price_editable ? <Unlock className="w-3 h-3"/> : <Lock className="w-3 h-3"/>}
                                </button>
                              </div>
                              {isZeroPrice && <div className="absolute -top-2 right-0 flex items-center gap-1 bg-red-100 text-red-600 text-[10px] px-1 rounded"><AlertTriangle className="w-3 h-3"/> Fiyat Giriniz</div>}
                            </td>
                            ...
                            <td className="p-2"><PriceInput value={item.unit_price} onChange={(val)=>handleItemChange(index, 'unit_price', val)} className="input-field text-right font-bold text-gray-800" /></td>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="card bg-gray-50 border border-gray-200">
                  ...
                  <div className="mt-6 pt-6 border-t border-gray-200 flex justify-end gap-3">
                    <button type="button" onClick={() => router.push('/quotes')} className="btn-secondary flex items-center gap-2"><X className="w-5 h-5"/> İptal</button>
                    <button type="submit" disabled={saving} className="btn-primary flex items-center gap-2">{saving ? 'Güncelleniyor...' : <><Save className="w-5 h-5"/> Güncelle</>}</button>
                  </div>
                </div>
              </div>
            </div>
